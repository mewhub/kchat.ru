...
$(document).ready(function(){
    ...
    $("<option value=''>Выберите пункт меню</option>").appendTo(dropdown);
...
$(function(){
  ...
  if(spaeva.el.length) {
    spaeva.dataembed = [];
    spaeva.dataembedmatch = [];
    spaeva.play = function(i) {
      spaeva.dataembed[i] = spaeva.el.eq(i).attr('onclick');
      spaeva.dataembedmatch[i] = spaeva.dataembed[i].match('youtube\.com\/embed\/([A-z0-9-_]+)\?');
      if(spaeva.dataembedmatch[i]) {
        /*$.ajax({
          url: 'https://gdata.youtube.com/feeds/api/videos/'+spaeva.dataembedmatch[i][1]+'?v=2&alt=jsonc',
          dataType: 'json',
          success: function(d){
            //d.data.title
            spaeva.el.eq(i)
              .css({'background':'#000 url(https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+encodeURI(d.data.thumbnail.hqDefault)+'&container=focus&gadget=a&rewriteMime=image/*&refresh=31536000&resize_w='+spaeva.el.eq(i).attr('data-embedw')+'&resize_h='+spaeva.el.eq(i).attr('data-embedh')+') center center no-repeat'})
              .html('<div style="background: url(https://ssl.gstatic.com/s2/tt/images/play-overlay.png) center center no-repeat; width: 100%; height: 100%;" title="Нажмите чтобы начать просмотр"></div>');
          },
        });*/
        spaeva.el.eq(i)
          .css({
            'background': '#000 url(https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url='+encodeURI('https://img.youtube.com/vi/'+spaeva.dataembedmatch[i][1]+'/hqdefault.jpg')+'&container=focus&gadget=a&rewriteMime=image/*&refresh=31536000&resize_w='+spaeva.el.eq(i).attr('data-embedw')+'&resize_h='+spaeva.el.eq(i).attr('data-embedh')+') center center no-repeat',
            'border': '1px solid #000',
          })
          .html('<div style="background: url(https://ssl.gstatic.com/s2/tt/images/play-overlay.png) center center no-repeat; width: 100%; height: 100%;" title="Нажмите чтобы начать просмотр"></div>');
      }
    };
    for(var i = 0; i < spaeva.el.length; i++) {
      spaeva.play(i);
    }
  }
...
c);c();return this}})(window.jQuery||window.Zepto);
  ...
  if($spoiler.length) {
    $spoiler.click(function(e){
      e.preventDefault();
      var $this = $(this);
      $this.next('div').toggle(300,function(){
        $(this).find('img.lazyload').trigger('unveil');
      });
      $this.text($this.text() === 'Скрыть текст' ? 'Показать скрытый текст' : 'Скрыть текст');
    });
  }
...
icms.comments = (function ($) {
    ...
    this.append = function(comment){
        ...
        if (comment.parent_id > 0){
            ...
            p.nextAll('.comment').each(function(){
                ...
                if ( $(this).data('level') <= p.data('level') || !$(this).data('level') ){
    ...
    this.remove = function (id){
        ...
        $.post(url, {id: id}, function(result){
            ...
            $('#comments_add_form').detach().prependTo('#comments_widget');
